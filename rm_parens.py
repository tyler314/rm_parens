...
class RemoveParens:
    ...
    def _set_open_parens_index(self):
        ...
        self._open_paren_index = self.line.index(open_paren_match) + len(open_paren_match) - 1 
    ...
    def _remove_parens(self):
        if self._unnecessary_parens():
            self.line = self.line[:self._open_paren_index] + self.line[self._open_paren_index+1:self._close_paren_index] + self.line[self._close_paren_index+1:]
    ...
    with open(sys.argv[1], 'w') as file:
        try:
            for line in file_lines:
                updated_line = RemoveParens(line)
                new_file_lines.append(updated_line.line)
